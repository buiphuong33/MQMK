...
class StanfordCars(datasets.StanfordCars):
    def __init__(self, root, split='train', transform=None, target_transform=None, download=False):
        ...
        self.targets= [sample[1] for sample in self._samples]
...
class Domainnet(torch.utils.data.Dataset):
    ...
    # 遍历目录树，从底部开始（即从叶子节点到根节点）
        for dirpath, dirnames, filenames in os.walk(directory, topdown=False):
            # 如果当前目录没有子目录和文件，说明是空目录
            if not dirnames and not filenames:
                try:
                    os.rmdir(dirpath)  # 删除空目录
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        ...
        self.target_transform=target_transform
        self.train = train
        self.fpath = os.path.join(root, 'domainnet')
        

        if not os.path.exists(self.fpath + '/train') or not os.path.exists(self.fpath + '/test'):
            self.dataset_clipart = datasets.ImageFolder(self.fpath+'/clipart/', transform=transform)
            self.dataset_infograph = datasets.ImageFolder(self.fpath+'/infograph/', transform=transform)
            self.dataset_painting = datasets.ImageFolder(self.fpath+'/painting/', transform=transform)
            self.dataset_quickdraw = datasets.ImageFolder(self.fpath+'/quickdraw/', transform=transform)
            self.dataset_sketch = datasets.ImageFolder(self.fpath+'/sketch/', transform=transform)
            self.dataset_real = datasets.ImageFolder(self.fpath+'/real/', transform=transform)
            # train_size = int(0.8 * len(self.dataset))
            # val_size = len(self.dataset) - train_size
            ...
            self.test_file_list = []
            with open(self.fpath+'/train.txt', 'r') as file:
                for line in file:
                    # 去除每行末尾的换行符，并按空格分割成列表
                    parts = line.strip().split()
                    
                    # 这里可以根据需要处理数据，比如转化为数值等
                    self.train_file_list.append(parts)

            with open(self.fpath+'/test.txt', 'r') as file:
                for line in file:
                    # 去除每行末尾的换行符，并按空格分割成列表
                    parts = line.strip().split()
                    
                    # 这里可以根据需要处理数据，比如转化为数值等
                    self.test_file_list.append(parts)
        ...
        if self.train:
            with open(self.fpath+'/train.txt', 'r') as file:
                    for line in file:
                        ...
                        # 这里可以根据需要处理数据，比如转化为数值等
                        label_map[parts[0].split('/')[-2]]=int(parts[1])
        ...
        if self.train:
            ...
            fpath = self.fpath + '/test'
        self.remove_empty_dirs(fpath)
        self.data = datasets.ImageFolder(fpath, transform=transform)

        self.data.class_to_idx=label_map
        for idx, (path, target) in enumerate(self.data.samples):
            # print(path.split('/'))
            self.data.samples[idx] = (path, label_map[path.split('/')[-2]])
            self.data.targets[idx] = label_map[path.split('/')[-2]]
        # self.data.samples.sort(key=lambda x: x[1])
        ...
        # self.data.samples = sorted(self.data.samples, key=lambda x: x[1])
    ...
    def split(self):
        ...
        for c in self.dataset_real.classes:
            if not os.path.exists(os.path.join(train_folder, c)):
                os.mkdir(os.path.join(os.path.join(train_folder, c)))
            if not os.path.exists(os.path.join(test_folder, c)):
                os.mkdir(os.path.join(os.path.join(test_folder, c)))
