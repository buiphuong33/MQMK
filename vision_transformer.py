...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False,perfect_match=False,
            use_g_prompt=False, g_prompt_length=None, g_prompt_layer_idx=None, use_prefix_tune_for_g_prompt=False,
            use_e_prompt=False, e_prompt_layer_idx=None, use_prefix_tune_for_e_prompt=False, same_key_value=False,multi_query=False,multi_key=False,k_key=1,class_group=1,using_new_classifier=False):
        """
        Args:
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            self.e_prompt = EPrompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
                    ...
                    num_heads=num_heads, same_key_value=same_key_value,multi_query=multi_query,multi_key=multi_key,class_per_task=math.ceil(num_classes/num_tasks),k_key=k_key,class_group=class_group)
        ...
        if using_new_classifier:
            self.ncm_head= nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.using_new_classifier=using_new_classifier

        self.embeddings=None
        self.targets=None
        self.mean_embeddings=[0 for _ in range(self.num_classes)]
        # print('mean_embeddings:',len(self.mean_embeddings)) 
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False, query=False,target=None):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_g_prompt or self.use_e_prompt:
                if self.use_prompt_mask and (train or self.perfect_match or query):
                # if self.use_prompt_mask:
                    start = task_id * self.e_prompt.top_k
                    # print('--------------------------------------')
                    # print('topk',self.e_prompt.top_k)
                    # print('--------------------------------------')
                    ...
                    if end > self.e_prompt.pool_size:
                        print('--------------------------------------')
                        print('end > self.e_prompt.pool_size,',prompt_mask)
                        print('--------------------------------------')
                        prompt_mask = None
                else:
                    prompt_mask = None
                ...
                g_prompt_counter = -1
                e_prompt_counter = -1
                if query==False and train==True and self.e_prompt.multi_key:
                    # print('--------------------------------------')
                    # print('query==False and train==True and self.e_prompt.multi_key')
                    # print('--------------------------------------')
                ...
                #     print('----------------------------------------------')
                #     # print('task_id.shape',task_id.shape)
                #     print('task_id:',task_id)
                #     # print('res.shape:',res['idx'].shape)
                #     print('res:',res['prompt_idx'])
                #     print('----------------------------------------------')

                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx and self.use_g_prompt:
    ...
    def new_classifier(self, res, pre_logits: bool = False):
        x = res['x']
        if self.class_token and self.head_type == 'token':
            if self.prompt_pool:
                x = x[:, self.total_prompt_len]
